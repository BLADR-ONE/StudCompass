...
const headerImagePageSlugs = HEADER_IMAGE_PAGE_CONFIGS.map(({ slug }) => slug);
const headerImagesShape = Object.fromEntries(
  headerImagePageSlugs.map((slug) => [slug, headerImageSchema.optional()]),
);
const headerImageHistoriesShape = Object.fromEntries(
  headerImagePageSlugs.map((slug) => [
    slug,
    z.array(headerImageSchema).max(HEADER_IMAGE_HISTORY_LIMIT).optional(),
  ]),
);
...
export const siteSettingsSchema = z.object({
  ...
  headerImages: z.object(headerImagesShape).optional(),
  recentHeaderImagesByPage: z.object(headerImageHistoriesShape).optional(),
});
...
export const adminFacultyUpsertSchema = z.object({
  ...
  emblemUrl: imageUrlSchema.nullable(),
